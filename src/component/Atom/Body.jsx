function Body({ newsData }) {
  ...
  const allData = newsData.map((el, i) => {
    return (
      ...
      <div key={i + "upper"} className="mb-6 flex flex-wrap bg-slate-900">
        <div
          key={i + "lower"}
        ...
        <div
          key={i + "c"}
          className=" mr-auto w-full shrink-0 grow-0 basis-auto px-3 md:mb-0 md:w-9/12 xl:w-7/12"
        >
          <a href={el.url}>
          ...
          <p className="mb-6 text-neutral-500 dark:text-neutral-300">
            <small>
              Published <u>{el.publishedAt.split("T")[0]}</u> <br /> by{" "}
              {" " + el.author}
            </small>
          </p>
